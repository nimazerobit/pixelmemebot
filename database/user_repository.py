...
class UserRepository:
    ...
    async def get(self, user_id: int) -> Optional[User]:
        async with aiosqlite.connect(self.db.path) as con:
            ...
            cur = await con.execute("SELECT user_id, full_name, banned, created_at FROM users WHERE user_id=?", (user_id,))
            row = await cur.fetchone()
            if row:
                return User(user_id=row['user_id'], full_name=row['full_name'], banned=bool(row['banned']), created_at=row['created_at'])
            return None
    ...
    async def get_users_page(self, limit: int, offset: int) -> list[User]:
        async with aiosqlite.connect(self.db.path) as con:
            ...
            rows = await cur.fetchall()
            return [User(user_id=row['user_id'], full_name=row['full_name'], banned=bool(row['banned']), created_at=row['created_at']) for row in rows]

    async def upsert(self, user_id: int, full_name: str = "") -> None:
        async with aiosqlite.connect(self.db.path) as con:
            await con.execute("INSERT OR IGNORE INTO users(user_id, full_name) VALUES (?, ?)", (user_id, full_name))
            await con.execute("UPDATE users SET full_name=? WHERE user_id=?", (full_name, user_id))
            await con.commit()
